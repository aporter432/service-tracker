...
class SimpleORBCOMMParser:
    ...
    def parse_text(self, text: str, subject: str = "", email_date: str = None) -> Dict:
        ...
        if subject:
            ...
            # Determine status
            if any(word in subject.lower() for word in ['resolved', 'completed', 'restored']):
        ...
        for line in lines:
            ...
            # Summary
                ...
                if summary_start != -1:
                    ...
                    time_match = re.search(r'(\d{1,2}:\d{2}\s*(?:UTC|GMT|EST|PST|[AP]M)?)', summary_text)
                    if time_match:
                        result['scheduled_time'] = time_match.group(1)
                    
                    # Look for duration
                    duration_match = re.search(r'(?:last|duration|take|approximately)\s+(\d+)\s+(hour|minute|day)s?', summary_text, re.IGNORECASE)
                    if duration_match:
                        result['duration'] = f"{duration_match.group(1)} {duration_match.group(2)}(s)"
                    
                    # Extract affected services
                    services = []
                    service_keywords = ['Partner-Support', 'VAPP', 'OGWS', 'Gateway', 'API', 'Portal', 'satellite', 'modem']
        ...
        if result['status'] == 'Resolved':
            ...
            if start_match:
                ...
                try:
                    ...
                    start_dt = datetime.strptime(start_time_str.replace(' GMT', ''), '%Y-%m-%d %H:%M')
                    result['incident_start_time'] = start_dt.strftime('%Y-%m-%d %H:%M:%S')
                    logger.debug(f"[{ref_num}] Parsed incident start time: {result['incident_start_time']}")
                except Exception as e:
                    # If parsing fails, leave as None
                    logger.warning(f"[{ref_num}] Failed to parse incident start time '{start_time_str}': {e}")
                    pass
            else:
                logger.debug(f"[{ref_num}] No incident start time found in resolved notification (older format)")
            ...
            if end_match:
                ...
                try:
                    # Parse format: "2025-10-22 15:05 GMT"
                    end_dt = datetime.strptime(end_time_str.replace(' GMT', ''), '%Y-%m-%d %H:%M')
                    result['incident_end_time'] = end_dt.strftime('%Y-%m-%d %H:%M:%S')
                    logger.debug(f"[{ref_num}] Parsed incident end time: {result['incident_end_time']}")
                except Exception as e:
                    # If parsing fails, leave as None
                    logger.warning(f"[{ref_num}] Failed to parse incident end time '{end_time_str}': {e}")
                    pass
            else:
                logger.debug(f"[{ref_num}] No incident end time found in resolved notification (older format)")

            # Calculate incident duration if both times are available
            if result['incident_start_time'] and result['incident_end_time']:
                try:
                    start_dt = datetime.strptime(result['incident_start_time'], '%Y-%m-%d %H:%M:%S')
                    end_dt = datetime.strptime(result['incident_end_time'], '%Y-%m-%d %H:%M:%S')
                    duration_delta = end_dt - start_dt
                    result['incident_duration_minutes'] = int(duration_delta.total_seconds() / 60)
                    logger.info(f"[{ref_num}] Calculated incident duration: {result['incident_duration_minutes']} minutes")
                except Exception as e:
                    # If calculation fails, leave as None
                    logger.error(f"[{ref_num}] Failed to calculate incident duration: {e}")
                    pass
            elif result['incident_start_time'] or result['incident_end_time']:
                # One time is present but not both
                logger.warning(f"[{ref_num}] Incomplete incident times - start: {result['incident_start_time']}, end: {result['incident_end_time']}")
    ...
    def export_to_csv(self, filename: str = 'orbcomm_notifications.csv'):
        ...
        fieldnames = [
            'reference_number', 'date_received', 'time_received', 'platform', 
            'event_type', 'status', 'scheduled_date', 'scheduled_time', 
            'duration', 'affected_services', 'summary'
        ]
    ...
    while True:
        ...
        if choice == '1':
            ...
            if text:
                ...
                print(f"\n✅ Added notification: {result.get('reference_number', 'No ref#')} - {result.get('event_type', 'Unknown event')}")
            ...
            sample_notifications = [
                {
                    ...
                    'body': '''Platform: IDP
Event: Maintenance
Summary: Dear ORBCOMM Partner, We will be conducting scheduled maintenance of the Partner-Support page and VAPP interface on November 5th at 15:00 UTC. The maintenance window is expected to last 1 hour.'''
                },
                {
                    'subject': 'ORBCOMM Service Notification: IDP-System Performance (Reference#: S-003141)-Open',
                    'body': '''Platform: IDP
Event: System Performance
Summary: Dear ORBCOMM Partner, We are currently experiencing system performance degradation affecting IDP services. Our team is investigating the issue.'''
                },
                {
                    'subject': 'ORBCOMM Service Notification: IDP-System Performance (Reference#: S-003141)-Resolved',
                    'body': '''Platform: IDP
Event: System Performance
Summary: Dear ORBCOMM Partner, The system performance issue has been resolved. All IDP services have been restored to normal operation.'''
                }
            ]
            ...
            filename = input("Enter output filename (default: orbcomm_notifications.csv): ").strip()
            if not filename:
