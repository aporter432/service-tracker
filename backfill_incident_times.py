...
def backfill_incident_times():
    ...
    for row in resolved_notifications:
        ...
        if incident_start and incident_end and incident_duration is not None:
            ...
            logger.info(f"✅ {reference_number}: {incident_duration} minutes ({incident_duration/60:.1f} hours)")
            updated_count += 1
        else:
            logger.warning(f"⚠️  {reference_number}: No incident times found in email body")
            skipped_count += 1
    ...
    if stats['total_pairs'] > 0:
        logger.info(f"  Paired notifications with incident times: {stats['total_pairs']}")
        logger.info(f"  Average email processing time: {stats['avg_email_time']:.1f} minutes ({stats['avg_email_time']/60:.1f} hours)")
        logger.info(f"  Average incident duration: {stats['avg_incident_time']:.1f} minutes ({stats['avg_incident_time']/60:.1f} hours)")

        time_difference = stats['avg_email_time'] - stats['avg_incident_time']
        logger.info(f"  Time difference (email - incident): {time_difference:.1f} minutes ({time_difference/60:.1f} hours)")
