...
class ORBCOMMParserApp:
    ...
    def setup_ui(self):
        ...
        file_menu.add_command(label="Open Email File...", command=self.load_email_file, accelerator="⌘O")
        file_menu.add_separator()
        file_menu.add_command(label="Save as CSV", command=self.save_to_csv, accelerator="⌘S")
        file_menu.add_command(label="Export to Excel", command=self.export_to_excel, accelerator="⌘E")
        file_menu.add_separator()
        ...
        edit_menu.add_command(label="Paste", command=self.paste_from_clipboard, accelerator="⌘V")
        edit_menu.add_command(label="Clear All", command=self.clear_all)
        edit_menu.add_command(label="Copy Results", command=self.copy_results, accelerator="⌘C")
        
        # Tools menu
        tools_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Tools", menu=tools_menu)
        tools_menu.add_command(label="Process Multiple Files", command=self.batch_process)
        ...
        # Title
        title_label = ttk.Label(main_frame, text="🛰️ ORBCOMM Service Notification Parser", 
        ...
        # Subject line input
        ttk.Label(input_frame, text="Subject Line (optional):").grid(row=0, column=0, sticky=tk.W)
        ...
        self.email_text = scrolledtext.ScrolledText(input_frame, height=10, wrap=tk.WORD,
                                                    font=('SF Mono', 11))
        ...
        # Styled buttons
        self.parse_btn = ttk.Button(button_frame, text="🔍 Parse Email", 
                                   command=self.parse_email, width=15)
        self.parse_btn.grid(row=0, column=0, padx=5)
        
        ttk.Button(button_frame, text="📋 Paste from Clipboard", 
                  command=self.paste_from_clipboard, width=20).grid(row=0, column=1, padx=5)
        
        ttk.Button(button_frame, text="📝 Load Sample", 
                  command=self.load_sample, width=15).grid(row=0, column=2, padx=5)
        
        ttk.Button(button_frame, text="🗑️ Clear All", 
                  command=self.clear_all, width=15).grid(row=0, column=3, padx=5)
        ...
        # Results treeview
        columns = ('Reference', 'Date', 'Platform', 'Event', 'Status', 'Scheduled', 'Duration', 'Services')
        ...
        ttk.Button(export_frame, text="💾 Save as CSV", 
                  command=self.save_to_csv, width=15).grid(row=0, column=0, padx=5)
        
        ttk.Button(export_frame, text="📊 Copy for Excel", 
                  command=self.copy_for_excel, width=15).grid(row=0, column=1, padx=5)
        
        ttk.Button(export_frame, text="📈 Open in Numbers", 
                  command=self.copy_to_numbers, width=15).grid(row=0, column=2, padx=5)
    ...
    def parse_email(self):
        ...
        if not body:
            messagebox.showwarning("No Input", "Please paste or type email content first!")
            return
        ...
        messagebox.showinfo("Success", f"Successfully parsed {len(emails)} notification(s)!")
    ...
    def parse_notification_text(self, text, subject):
        ...
        duration_match = re.search(r'(?:last|duration)\s+(\d+)\s+(hour|minute)s?', summary_text, re.I)
        if duration_match:
            result['duration'] = f"{duration_match.group(1)} {duration_match.group(2)}(s)"
        
        # Extract services
        services = []
        service_keywords = ['Partner-Support', 'VAPP', 'OGWS', 'Gateway', 'API', 'Portal']
    ...
    def load_sample(self):
        """Load sample data."""
        sample_subject = "ORBCOMM Service Notification: IDP-Maintenance (Reference#: M-003147)-Open"
...
Summary: Dear ORBCOMM Partner, We will be conducting scheduled maintenance of the Partner-Support page and VAPP interface on November 5th at 15:00 UTC. The maintenance window is expected to last 1 hour."""
    ...
    def save_to_csv(self):
        ...
        if filename:
            ...
            if messagebox.askyesno("Open File", "Do you want to reveal the file in Finder?"):
    ...
    def open_in_vscode(self):
        ...
        try:
            ...
            messagebox.showerror("Error", "Could not open VS Code. Is it installed and in PATH?")
